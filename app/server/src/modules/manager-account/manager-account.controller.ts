...
// Controller lấy danh sách tài khoản quản lý (có lọc/phân trang từ query string)
...
// Controller lấy chi tiết 1 tài khoản theo Id
...
// Controller tạo mới tài khoản quản lý
export const create = asyncHandler(async (req: Request, res: Response) => {
  const body = req.body as CreateManagerAccountInput;
  const account = await managerAccountService.create(body);
  res.status(HttpStatus.CREATED).json(account); // Trả về 201 kèm tài khoản vừa tạo
});

// Controller cập nhật thông tin tài khoản theo Id
export const update = asyncHandler(async (req: Request, res: Response) => {
  const body = req.body as UpdateManagerAccountInput;
  const account = await managerAccountService.update(
...
// Controller đặt lại mật khẩu cho tài khoản theo Id
...
// Controller xóa tài khoản theo Id

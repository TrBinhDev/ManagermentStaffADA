...
// Controller lấy lịch làm việc của chính người đang đăng nhập
export const listWorkSchedule = asyncHandler(
  async (req: Request, res: Response) => {
    const query = req.query as unknown as ListEmployeeWorkScheduleQuery;
    // employeeId lấy từ token (req.user), KHÔNG lấy từ param/query -> không thể xem lịch của người khác
    const result = await meService.listWorkSchedule(
      req.user!.employeeId!,
      query,
    );
...
// Controller lấy lịch sử chấm công của chính người đang đăng nhập
...
// Controller lấy lương của chính người đang đăng nhập
...
// Controller lấy hồ sơ cá nhân của chính người đang đăng nhập
export const getProfile = asyncHandler(async (req: Request, res: Response) => {
  const result = await meService.getProfile(req.user!.employeeId!);
  res.status(HttpStatus.OK).json(result);
});

// Controller tự cập nhật hồ sơ cá nhân của chính người đang đăng nhập
